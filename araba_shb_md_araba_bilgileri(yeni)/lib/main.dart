...
void main() {
  ...
  runApp( ChangeNotifierProvider(
    create: (context) => ArabaDepo(),
    child: const arabaSahibi(),
  ),
  );
}
...
class _BilgiYazState extends State<BilgiYaz> {
  ...
  Future<void> kaskoData(String kaskono) async {
    try {
      var url = Uri.parse(
          'http://***********/kaskokontrol'); // API endpoint'i
      var body = jsonEncode( {
  ...
  @override
  Widget build(BuildContext context) {
    var arabashbProvider = Provider.of<ArabaDepo>(context, listen: false);
    return Scaffold(
      ...
      Visibility(
        ...
        ElevatedButton(
            onPressed: () async {
              ...
              if (ruhsatkontrol == "" && kaskokontrol == "" && _ibanController.text.length == 26) {
